...
**Delay.** A conversational proxy must wait a substantial period — on the order of a day, configurable by the operator — before considering a response to any given post or thread. If a human provides a satisfactory response in that window, the proxy stands down. The delay is not a rate-limiting technicality; it is the mechanism that keeps the proxy in a genuinely secondary role.
...
**Epistemic hedging is mandatory, not stylistic.** A conversational proxy speaks in probabilistic, attributive language — "I think X would probably say...", "according to [source]...", "I could be wrong, but..." — never in its operator's own voice as though it *were* them. It represents a prediction about what its operator might think, not a substitute utterance on their behalf.

**Optional social ping.** Rather than attempting to speak for the operator at all, a proxy can instead simply flag the operator directly — "this looks like a conversation you might have something to say about" — optionally with a tentative, clearly-hedged summary of what it predicts the operator's view would be, explicitly framed as provisional: "I think @user would probably say [tentative summary], but I'll let them elaborate if they want to." This keeps the proxy in a purely advisory role with respect to the actual conversation, deferring entirely to the human's own words if and when they arrive.

**Operator-invited elaboration.** A distinct, more deliberate use case: a human participant can explicitly invite their own proxy to attempt an elaboration of their view before they weigh in themselves — "let my proxy try first, and I'll correct anything it gets wrong." This differs from the autonomous delayed-response mode in that it is directly solicited rather than independently triggered, and it produces especially valuable training signal, since the operator's corrections are responding to a concrete, specific attempt rather than a hypothetical.
